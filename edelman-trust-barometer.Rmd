...
edelman_2019 <- read_excel("~/Desktop/データセット/edelman-trust-barometer.xlsx", sheet = 1) %>% select(country, starts_with("2019_")) %>% mutate(country = str_to_title(country))

edelman_2020 <- read_excel("~/Desktop/データセット/edelman-trust-barometer.xlsx", sheet = 2) %>% mutate(country = str_to_title(country))
```
...
edelman_2020 %>% 
  ...
  mutate(category = fct_recode(category, "Jan 2019" = "jan_gov", "May 2019" = "may_gov")) %>% 
